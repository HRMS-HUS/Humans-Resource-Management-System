...
async def get_all_events(db: AsyncSession, skip: int = 0, limit: int = 100) -> List[models.UserPersonalEvent]:
    result = await db.execute(
        select(models.UserPersonalEvent)
        .offset(skip)
        .limit(limit)
    )
    return result.scalars().all()

async def update_user_event(db: AsyncSession, event_id: str, event: schemas.UserPersonalEventUpdate):
    try:
        query = update(models.UserPersonalEvent).where(
            models.UserPersonalEvent.event_id == event_id
